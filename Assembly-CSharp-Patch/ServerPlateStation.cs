...
public class ServerPlateStation : ServerSynchroniserBase
{
	...
	{
		// patch
		bool isTray = m_plate != null && m_plate.GetComponent<Tray>() != null;
		int num = m_serverReturnStations.FindIndex_Predicate((ServerPlateReturnStation x) =>
		{
            if (x.GetPlatingStep() != _plateType) return false;
            PlateReturnStation plateReturnStation = x.GetComponent<PlateReturnStation>();
            if (plateReturnStation == null || plateReturnStation.m_stackPrefab == null) return false;
            PlateStackBase plateStackBase = plateReturnStation.m_stackPrefab.GetComponent<PlateStackBase>();
			if (plateStackBase == null) return false;
			GameObject plate = plateStackBase.m_platePrefab;
			if (plateStackBase is DirtyPlateStack)
			{
				DirtyPlateStack dirtyPlateStack = plateStackBase as DirtyPlateStack;
				if (dirtyPlateStack.m_washedPrefab != null)
					plate = dirtyPlateStack.m_washedPrefab.GetComponent<CleanPlateStack>().m_platePrefab;
			}
			bool isTray2 = plate != null && plate.GetComponent<Tray>() != null;
            return isTray == isTray2;
        });
		// patch
		if (num >= 0)
		{
