...
public class ServerWashingStation : ServerSynchroniserBase, IHandlePlacement, IBaseHandlePlacement
{
	...
	private void OnItemAdded(IAttachment _attachment)
	{
		...
		// patch
		// can't wash clean stack or stack of wrong type
		//ServerStack component = gameObject.GetComponent<ServerStack>();
		DirtyPlateStack dirtyPlateStack = gameObject.GetComponent<DirtyPlateStack>();
        if (dirtyPlateStack != null)
        {
            GameObject m_stackPrefab = m_washingStation.m_dryingStation.m_stackPrefab;
		...
		ServerDirtyPlateStack component = gameObject.GetComponent<ServerDirtyPlateStack>();
        // patch
		if (component != null)
		{
